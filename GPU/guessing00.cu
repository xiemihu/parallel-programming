...
void threadMain(PriorityQueue *q, const string &guess, segment *a, int total_tasks) {
    ...
    for (int base = 0; base < total_tasks; base += CHUNK) {
        ...
        CUDA_CHECK(cudaMalloc((void **)&d_guess, h_guess.size() * sizeof(char)));
        CUDA_CHECK(cudaMalloc((void **)&d_values, h_values.size() * sizeof(char)));
        CUDA_CHECK(cudaMalloc((void **)&d_out, h_out.size() * sizeof(char)));

        CUDA_CHECK(cudaMemcpy(d_guess, h_guess.data(), h_guess.size() * sizeof(char), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_values, h_values.data(), h_values.size() * sizeof(char), cudaMemcpyHostToDevice));
        ...
        CUDA_CHECK(cudaMemcpy(h_out.data(), d_out, h_out.size() * sizeof(char), cudaMemcpyDeviceToHost));

        for (int i = 0; i < now; ++i) {
            q->guesses[gbegin + (size_t)base + (size_t)i] =
                string(&h_out[(size_t)i * out_stride]);
        }
