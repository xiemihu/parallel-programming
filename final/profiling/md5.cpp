...
void MD5Hash_NEON(const string input[4], bit32 state[4][4])
{
    ...
    if (single_block)
    {
        for (int i = 0; i < 4; i += 1)
        {
            ...
            memset(shortMessage[i], 0, 64);
            memcpy(shortMessage[i], input[i].data(), length);

            shortMessage[i][length] = 0x80;
            ...
            paddedMessage[i] = shortMessage[i];
            messageLength[i] = 64;
        }
    }
    else
    {
        paddedMessage[0] =
            StringProcess(input[0], &messageLength[0]);
        paddedMessage[1] =
            StringProcess(input[1], &messageLength[1]);
        paddedMessage[2] =
            StringProcess(input[2], &messageLength[2]);
        paddedMessage[3] =
            StringProcess(input[3], &messageLength[3]);
    }
