...
#pragma omp section
        {
            MergeSegments(letters, letters_freq, letters_ids, letters_id, other_model.letters, other_model.letters_freq);
        }

#pragma omp section
        {
            MergeSegments(digits, digits_freq, digits_ids, digits_id, other_model.digits, other_model.digits_freq);
        }

#pragma omp section
        {
            MergeSegments(symbols, symbols_freq, symbols_ids, symbols_id, other_model.symbols, other_model.symbols_freq);
        }
    }
}

void model::trainBatch(const vector<string> &passwords)
{
...
#pragma omp single
        ...
        int begin = (long long)passwords.size() * tid / active_threads;
        int end = (long long)passwords.size() * (tid + 1) / active_threads;
...
void model::train(const string &path)
{
    ...
    cout << "Training..." << endl;
    cout << "Training phase 1: reading and parsing passwords..."
         << endl;
    ...
    while (train_set >> pw)
    {
        ...
        passwords.emplace_back(std::move(pw));

        if ((int)passwords.size() == train_batch_num)
        {
            trainBatch(passwords);
            passwords.clear();
        }
    }
