...
long long hashLocalGuesses(const vector<string> &guesses, const unordered_set<string> &test_set, double &local_hash_time)
{
    ...
    for (int i = 0; i < hash_count; i += 4)
    {
        ...
        if (test_set.find(guesses[i]) != test_set.end()) local_cracked += 1;
        if (test_set.find(guesses[i + 1]) != test_set.end()) local_cracked += 1;
        if (test_set.find(guesses[i + 2]) != test_set.end()) local_cracked += 1;
        if (test_set.find(guesses[i + 3]) != test_set.end()) local_cracked += 1;
    }
